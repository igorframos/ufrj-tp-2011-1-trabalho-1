...
namespace comunicacao
{
    ...
    bool recebeNomeUsuario(Controle* controle, Cliente& cliente)
    {
        ...
        while (nomeInvalido)
        {    
            if (read(cliente.socketEnvioID, nome, sizeof(nome)) < 0)    // Recebe o nome desejado pelo cliente.
            {
                std::cout << "Falha ao receber nome do cliente." << std::endl;
                return false;
            }
    ...
    void recebeMensagens(Controle* controle, Cliente& cliente)
    {
        ...
        while (!(controle->sair))
        {
            ...
            if (read(cliente.socketEnvioID, &mensagem, sizeof(mensagem)) < 0)  // Recebe uma mensagem do cliente.
            {
                strcpy(mensagem.remetente, cliente.nome.c_str());
                strcpy(mensagem.texto, "exit_program");
            }

            if (!strlen(mensagem.remetente) || !strlen(mensagem.texto))
            {
                strcpy(mensagem.remetente, cliente.nome.c_str());
                strcpy(mensagem.texto, "exit_program");
            }
